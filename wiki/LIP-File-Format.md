# KotOR [LIP](LIP-File-Format) file format Documentation

LIP ([LIP](LIP-File-Format) Synchronization) files drive mouth [animation](MDL-MDX-File-Format#animation-header) for voiced dialogue. Each file contains a compact series of [keyframes](MDL-MDX-File-Format#controller-structure) that map timestamps to discrete viseme (mouth shape) indices so that the engine can interpolate character [LIP](LIP-File-Format) movement while playing the companion [WAV](WAV-File-Format) line.
...
- [LIP files](LIP-File-Format) [ARE](GFF-File-Format#are-area) always binary (`"LIP V1.0"` signature) and contain only [animation](MDL-MDX-File-Format#animation-header) data.  
- They [ARE](GFF-File-Format#are-area) paired with [WAV](WAV-File-Format) voice-over resources of identical duration; the [LIP](LIP-File-Format) `length` field must match the [WAV](WAV-File-Format) `data` playback time for glitch-free [animation](MDL-MDX-File-Format#animation-header).  
...
- [`vendor/xoreos/src/graphics/aurora/lipfile.cpp`](https://github.com/th3w1zard1/xoreos/blob/master/src/graphics/aurora/lipfile.cpp) - Generic Aurora [LIP](LIP-File-Format) implementation (shared format)
...
- [WAV File Format](WAV-File-Format) - Audio format paired with [LIP](LIP-File-Format) files
