...
This layout is identical across PyKotor, Reone, Xoreos, KotOR.js, and the original Bioware tools; KotOR-Unity and NorthernLights consume the same [header](GFF-File-Format#file-header).
...
- [`vendor/xoreos/src/graphics/images/tpc.cpp`](https://github.com/th3w1zard1/xoreos/blob/master/src/graphics/images/tpc.cpp) - Generic Aurora [TPC](TPC-File-Format) implementation (shared [format](GFF-File-Format))
...
- [GFF-GUI](GFF-GUI) - [GUI](GFF-File-Format#gui-graphical-user-interface) [files](GFF-File-Format) that reference [TPC](TPC-File-Format) [textures](TPC-File-Format) for UI elements
...
- PyKotor normalizes cube [faces](MDL-MDX-File-Format#face-structure) after reading (deswizzle + [rotation](MDL-MDX-File-Format#node-header)) so that [face](MDL-MDX-File-Format#face-structure) ordering matches the high-level [texture](TPC-File-Format) API.  
