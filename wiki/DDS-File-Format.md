...
DirectDraw Surface (DDS) textures appear in two flavours across KotOR-era content:
...
- Magic: `DDS` followed by a 124-[byte](GFF-File-Format#byte) [header](GFF-File-Format#file-header).
- Important [header](GFF-File-Format#file-header) [fields](GFF-File-Format#file-structure):
  - `dwFlags` [bit](GFF-File-Format#data-types) `0x00020000` signals mipmap [count](GFF-File-Format#file-structure); otherwise one mipmap is assumed.
  ...
  - `DDPIXELFORMAT` describes layout:
    ...
    - Uncompressed 32-[bit](GFF-File-Format#data-types) BGRA masks (`00FF0000/0000FF00/000000FF/FF000000`) → TPC `BGRA`
    - Uncompressed 24-[bit](GFF-File-Format#data-types) BGR masks (`00FF0000/0000FF00/000000FF`) → TPC `BGR`
    - 16-[bit](GFF-File-Format#data-types) ARGB 1-5-5-5 (`7C00/03E0/001F/8000`) → converted to RGBA
    - 16-[bit](GFF-File-Format#data-types) ARGB 4-4-4-4 (`0F00/00F0/000F/F000`) → converted to RGBA
    - 16-[bit](GFF-File-Format#data-types) RGB 5-6-5 (`F800/07E0/001F/0`) → converted to RGB
  - Cubemap detection via `dwCaps2 & 0x00000200`; faces counted from `dwCaps2 & 0x0000FC00`.
- Each mip [size](GFF-File-Format#file-structure) is computed with the [format](GFF-File-Format)-aware block sizing from `TPCTextureFormat.get_size`.
- [data](GFF-File-Format#file-structure) layouts that are not directly usable (4444, 1555, 565) are expanded into RGBA/RGB before storing in the `TPC` object.

Implementation reference:

- `Libraries/PyKotor/src/pykotor/resource/formats/tpc/io_dds.py` (standard DDS path and [format](GFF-File-Format) mapping)
- `vendor/xoreos/src/graphics/images/dds.cpp` and `vendor/xoreos-tools/src/images/dds.cpp` (baseline behaviour and [mask](GFF-File-Format#data-types) checks)

### BioWare DDS variant

- No magic; [header](GFF-File-Format#file-header) is four [little-endian](https://en.wikipedia.org/wiki/Endianness) [uint32](GFF-File-Format#dword) [values](GFF-File-Format#data-types):
...
- Followed by an unused [float](GFF-File-Format#float), then the compressed payload and inferred mip levels until [data](GFF-File-Format#file-structure) is exhausted.
- Mipmap [count](GFF-File-Format#file-structure) is inferred by walking expected block sizes until [data](GFF-File-Format#file-structure) would underflow.
...
- `TPCDDSWriter` emits only standard DDS [headers](GFF-File-Format#file-header):
  ...
  - Non-DDS-friendly [formats](GFF-File-Format) are converted (`RGB`→`BGR`, `RGBA`→`BGRA`).
...
- **TGA**: uncompressed/RLE raster [data](GFF-File-Format#file-structure); no block compression; single face only; origin/alpha [flags](GFF-File-Format#data-types) live in the [header](GFF-File-Format#file-header). DDS can be block-compressed (DXT1/3/5) and include cubemap faces/mip hierarchies in one container.
- **TPC**: KotOR-specific container with TXI embedded and different [header](GFF-File-Format#file-header) layout; PyKotor maps DDS surfaces into `TPC` objects for unified downstream handling (conversion, TXI logic, cubemap normalization).
