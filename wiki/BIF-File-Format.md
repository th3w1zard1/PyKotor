# KotOR [BIF](BIF-File-Format) file format Documentation

This document provides a detailed description of the BIF (BioWare index file) file format used in Knights of the Old Republic (KotOR) games. [BIF files](BIF-File-Format) [ARE](GFF-File-Format#are-area) [archive containers](ERF-File-Format) that store the bulk of game resources.
...
[BIF files](BIF-File-Format) work in tandem with [KEY files](KEY-File-Format) which provide the filename-to-resource mappings. [BIF files](BIF-File-Format) contain only resource IDs, types, and data - the actual filenames ([ResRefs](GFF-File-Format#gff-data-types)) [ARE](GFF-File-Format#are-area) stored in the [KEY file](KEY-File-Format). BIF files [ARE](GFF-File-Format#are-area) [archive containers](ERF-File-Format) that store the bulk of game resources.
...
- [KEY File Format](KEY-File-Format) - index file that references [BIF archives](BIF-File-Format)
...
| ------------------------- | ------- | ------ | ---- | ---------------------------------------------- |
| file type                 | [char](GFF-File-Format#gff-data-types) | 0 (0x00) | 4    | `"BIFF"` for [BIF](BIF-File-Format), `"BZF "` for compressed [BIF](BIF-File-Format)  |
| file Version              | [char](GFF-File-Format#gff-data-types) | 4 (0x04) | 4    | `"V1  "` for [BIF](BIF-File-Format), `"V1.0"` for BZF             |
...
**Reference**: [`vendor/xoreos-docs/specs/torlack/bif.html`](https://github.com/th3w1zard1/xoreos-docs/blob/master/specs/torlack/bif.html) - Tim Smith (Torlack)'s reverse-engineered [BIF](BIF-File-Format) format documentation
...
| Name        | type   | offset | size | Description                                                      |
| ----------- | ------ | ------ | ---- | ---------------------------------------------------------------- |
| Resource ID | [uint32](GFF-File-Format#gff-data-types) | 0 (0x00) | 4    | Resource ID (matches [KEY file](KEY-File-Format) entry, encodes [BIF](BIF-File-Format) index and resource index) |
...
- Resource data is stored in its native format (no additional [BIF](BIF-File-Format)-specific wrapping or metadata)
...
2. **ID Decoding**: Extract the [BIF](BIF-File-Format) index (upper 12 bits) and resource index (lower 20 bits) from the Resource ID
3. **BIF Selection**: Use the [BIF](BIF-File-Format) index to identify which [BIF file](BIF-File-Format) contains the resource
4. **Table Access**: Read the [BIF file](BIF-File-Format) header to find the offset to the variable resource table
...
The Resource ID in the [BIF file](BIF-File-Format)'s variable resource table must match the Resource ID stored in the [KEY file](KEY-File-Format). The Resource ID is a 32-bit value that encodes two pieces of information:

- **Lower 20 bits (bits 0-19)**: Resource index within the [BIF](BIF-File-Format) file (0-based index into the variable resource table)
- **Upper 12 bits (bits 20-31)**: [BIF](BIF-File-Format) index in the [KEY file](KEY-File-Format)'s [BIF](BIF-File-Format) table (identifies which [BIF file](BIF-File-Format) contains this resource)

**Example:** A Resource ID of `0x00400029` decodes as:

- Resource index: `0x29` (41st resource in the [BIF](BIF-File-Format))
- [BIF](BIF-File-Format) index: `0x004` (4th [BIF file](BIF-File-Format) in the [KEY](KEY-File-Format)'s [BIF](BIF-File-Format) table)
...
BZF files [ARE](GFF-File-Format#are-area) LZMA-compressed [BIF files](BIF-File-Format) used primarily in iOS (and maybe Android) ports of KotOR. The BZF header contains `"BZF "` + `"V1.0"`, followed by LZMA-compressed [BIF](BIF-File-Format) data. Decompression reveals a standard [BIF](BIF-File-Format) structure.
...
- The entire [BIF](BIF-File-Format) file (after the 8-[byte](GFF-File-Format#gff-data-types) header) is compressed using LZMA (Lempel-Ziv-Markov chain Algorithm)
...
- Modding tools can (and should) convert between [BIF](BIF-File-Format) and BZF formats freely

**Implementation Notes:**

The BZF wrapper is completely transparent to the game engine - once decompressed in memory, the resource access patterns [ARE](GFF-File-Format#are-area) identical to standard [BIF files](BIF-File-Format). Tools should decompress BZF files before reading resource entries, as the variable resource table offsets [ARE](GFF-File-Format#are-area) relative to the decompressed [BIF](BIF-File-Format) structure.
...
[BIF](BIF-File-Format) files require a [KEY file](KEY-File-Format) to map resource IDs to filenames (ResRefs). The [KEY file](KEY-File-Format) contains:

- [BIF](BIF-File-Format) file entries (filename, size, location)
...
This documentation aims to provide a comprehensive overview of the KotOR [BIF file](BIF-File-Format) format, focusing on the detailed file structure and data formats used within the games.
