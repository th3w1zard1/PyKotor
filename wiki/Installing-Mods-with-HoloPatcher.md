...
**Choose Installation Option:** If the mod provides multiple installation options (indicated by a namespaces.ini [file](GFF-File-Format)), select your preferred option from the first dropdown menu.
...
**Navigate to Tools -> Uninstall Mod/Restore Backup from the top menu.** This action restores your game [files](GFF-File-Format) to their state just before the latest mod installation, effectively undoing all recent changes.
...
**Partial Installations:** If an installation is interrupted (for example, if the app is closed prematurely) and then restarted, the mod is reapplied over partially modified [files](GFF-File-Format). This can result in duplication within critical game [files](GFF-File-Format), such as [appearance.2da](2DA-appearance), leading to potential game-breaking issues.

**Correcting Mistakes:** If you inadvertently install a mod multiple times without reverting, the game [files](GFF-File-Format) may contain redundant modifications. However, HoloPatcher keeps backups for each installation. To correct this, you must use the Uninstall Mod/Restore Backup feature twice:

The first execution removes the modifications from the most recent (second) installation, reverting [files](GFF-File-Format) to their state after the first (interrupted) installation.
The second execution then removes the remaining modifications, fully reverting your game [files](GFF-File-Format) to their original state before any installation attempts.

## Installing Mods on iOS Devices

For iOS installations, it's critical to ensure that all KotOR [file](GFF-File-Format) names [ARE](GFF-File-Format#are-area) in lowercase. If [file](GFF-File-Format) names retain uppercase characters, the game will crash immediately after tapping the 'play' button on the main menu.
