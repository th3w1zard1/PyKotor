...
KotOR stores both standard [WAV](WAV-File-Format) voice-over lines and Bioware-obfuscated sound-effect [files](GFF-File-Format). Voice-over assets [ARE](GFF-File-Format#are-area) regular RIFF containers with PCM [headers](GFF-File-Format#file-header), while SFX assets prepend a 470-[byte](GFF-File-Format#gff-data-types) custom block before the RIFF [data](GFF-File-Format#file-structure-overview). PyKotor handles both variants transparently.
...
## [file](GFF-File-Format) [types](GFF-File-Format#gff-data-types)
...
| **SFX (Sound effects)** | Combat, UI, ambience, `.wav` [files](GFF-File-Format) under `StreamSounds`/`SFX`. | Contains a Bioware 470-[byte](GFF-File-Format#gff-data-types) obfuscation [header](GFF-File-Format#file-header) followed by the same RIFF [data](GFF-File-Format#file-structure-overview). |
...
## Standard RIFF/WAVE [structure](GFF-File-Format#file-structure-overview)

KotOR sticks to the canonical RIFF chunk order:

| [offset](GFF-File-Format#file-structure-overview) | [field](GFF-File-Format#file-structure-overview) | Description |
| ------ | ----- | ----------- |
| 0 (0x00) | `"RIFF"` | Chunk ID |
| 4 (0x04) | `<uint32>` | [file](GFF-File-Format) [size](GFF-File-Format#file-structure-overview) minus 8 |
...
| [field](GFF-File-Format#file-structure-overview) | [type](GFF-File-Format#data-types) | Description |
| ----- | ---- | ----------- |
...
### [data](GFF-File-Format#file-structure-overview) Chunk
...
| `<byte[]>` | PCM/ADPCM sample [data](GFF-File-Format#file-structure-overview). |

KotOR voice-over WAVs add a `"fact"` chunk with a 32-[bit](GFF-File-Format#gff-data-types) sample [count](GFF-File-Format#file-structure-overview), which PyKotor writes for compatibility (`io_wav.py:182-186`).
...
- **PCM (`audio_format = 0x0001`)**: Most dialogue is 16-[bit](GFF-File-Format#gff-data-types) mono PCM, which streams directly through the engine mixer.  
...
- KotOR requires `block_align` and `bytes_per_sec` to match the [values](GFF-File-Format#gff-data-types) implied by the codec; mismatched [headers](GFF-File-Format#file-header) can crash the in-engine decoder.  
...
- **[data](GFF-File-Format#file-structure-overview) [model](MDL-MDX-File-Format):** [`Libraries/PyKotor/src/pykotor/resource/formats/wav/wav_data.py`](https://github.com/th3w1zard1/PyKotor/blob/master/Libraries/PyKotor/src/pykotor/resource/formats/wav/wav_data.py)  
...
With this [structure](GFF-File-Format#file-structure-overview), [WAV](WAV-File-Format) assets authored in PyKotor will play identically in the base game and in the other vendor tools.
