...
- If the scripts [ARE](GFF-File-Format#are-area) not specified, default ones will be used.
...
- `51. EffectAssuredHit`
- EffectAssuredHit
- Create an Assured Hit effect, which guarantees that all attacks [ARE](GFF-File-Format#are-area) successful
...
- Effect that will play an [animation](MDL-MDX-File-Format#animation-header) and display a visual effect to indicate the
...
- If multiple targets [ARE](GFF-File-Format#are-area) specified, then the lightsaber travels to them
...
- effect will cause an effect icon to appear on the portrait/charsheet [GUI](GFF-File-Format#gui-graphical-user-interface).
