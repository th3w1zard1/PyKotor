	...
	if ((nGlobal > (-1))) {
		...
		if ((float18 > 3800.0)) {
			...
			if ((nGlobal < 6)) {
				...
				AssignCommand(GetFirstPC(), DelayCommand(3.0, StartNewModule("tat_m17ae", "tat17ae_swoop_return", "", "", "", "", "", "")));
			}
		}
		else {
			...
			SetGlobalNumber("TAT_SWOOP_MIN", (int33 / 60));
			SetGlobalNumber("TAT_SWOOP_SEC", FloatToInt((((IntToFloat(int33) / 60.0) - (int33 / 60)) * 60.0)));
			SetGlobalNumber("TAT_SWOOP_MSEC", FloatToInt(((float16 - int33) * 100.0)));
