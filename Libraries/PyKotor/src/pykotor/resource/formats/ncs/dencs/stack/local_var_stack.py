...
class LocalVarStack(LocalStack):
    def __init__(self):
        ...
        self.stack: list[StackEntry] | None = []

    def _require_stack(self) -> list[Self]:
        if self.stack is None:
            raise RuntimeError("Stack has been disposed")
        return self.stack
    ...
    def size(self) -> int:
        size = 0
        for entry in self._require_stack():
            size += entry.size()
        return size
    ...
    def structify(self, firstelement: int, count: int, subdata: SubroutineAnalysisData) -> VarStruct | None:
        pos = 0
        stack_list = list(self._require_stack())
    ...
    def clone(self):
        ...
        new_stack.stack = list(self._require_stack())
        for entry in new_stack.stack:
