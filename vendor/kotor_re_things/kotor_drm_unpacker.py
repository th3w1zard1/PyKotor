...
class ProcessMemoryReader:
    ...
    def _open_process(self):
        ...
        self.handle = kernel32.OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, False, self.process_id)
        if not self.handle:
    ...
    def read_memory(self, address: int, size: int) -> bytes:
        ...
        success = kernel32.ReadProcessMemory(self.handle, ctypes.c_void_p(address), buffer, size, ctypes.byref(bytes_read))
...
class KotorUnpacker:
    ...
    def dump_process(self, process: psutil.Process, output_path: Path) -> bool:
        ...
        with ProcessMemoryReader(process.pid) as reader:
            ...
            # Read section headers
            section_headers = reader.read_memory(image_base + section_header_offset, num_sections * 0x28)
...
class KotorPacker:
    ...
    def _is_text_encrypted(self) -> bool:
        ...
        has_decrypted_prologue = (
            (decrypted_bytes[0] == 0x55 and len(decrypted_bytes) > 1 and decrypted_bytes[1] in [0x8B, 0x89])  # push ebp; mov
            or (decrypted_bytes[0] == 0x6A and len(decrypted_bytes) > 1)  # push imm8
            or (decrypted_bytes[0] == 0xE8 and len(decrypted_bytes) > 4)  # call
        )

        has_encrypted_prologue = (
            (encrypted_bytes[0] == 0x55 and len(encrypted_bytes) > 1 and encrypted_bytes[1] in [0x8B, 0x89])
            or (encrypted_bytes[0] == 0x6A and len(encrypted_bytes) > 1)
            or (encrypted_bytes[0] == 0xE8 and len(encrypted_bytes) > 4)
        )

        # Encrypted if: decrypted has prologue but encrypted doesn't, OR decrypted has more valid instructions
        return bool((has_decrypted_prologue and not has_encrypted_prologue) or (decrypted_valid > encrypted_valid + 2))
    ...
    def pack(self, output_path: Path, encrypt_text: bool = True) -> bool:
        ...
        stub = self._create_unpacker_stub(original_oep, text_section.VirtualAddress, text_section.SizeOfRawData)
        ...
        try:
            ...
            if current_ep != original_oep:
                print("Warning: Current entry point mismatch, searching...")
    ...
    def unpack(self, output_path: Path) -> bool:
        ...
        if is_packed:
            ...
            stub_section = None
            for section in self.pe.sections:
                if section.VirtualAddress <= current_entry < section.VirtualAddress + section.Misc_VirtualSize:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="KOTOR DRM Unpacker and Packer", formatter_class=argparse.RawDescriptionHelpFormatter, epilog=__doc__)
