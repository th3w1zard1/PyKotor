...
class KotorREParser:
    ...
    def _parse_function_element(self, elem) -> Optional[FunctionInfo]:
        ...
        if stack_frame is not None:
            for stack_var in stack_frame.findall("STACK_VAR"):
                var_info = {"name": stack_var.get("NAME"), "offset": stack_var.get("STACK_PTR_OFFSET"), "datatype": stack_var.get("DATATYPE"), "size": stack_var.get("SIZE")}
        ...
        for reg_var in elem.findall("REGISTER_VAR"):
            func.register_vars.append({"name": reg_var.get("NAME"), "register": reg_var.get("REGISTER"), "datatype": reg_var.get("DATATYPE")})
    ...
    def _parse_data_element(self, elem) -> Optional[DataDefinition]:
        ...
        return DataDefinition(address=addr_int, datatype=datatype, datatype_namespace=elem.get("DATATYPE_NAMESPACE"), size=size_int)
    ...
    def _parse_comment_element(self, elem) -> Optional[CommentInfo]:
        ...
        return CommentInfo(address=addr_int, comment_type=comment_type or "end-of-line", text=text.strip())
    ...
    def _parse_header(self):
        ...
        for match in struct_pattern.finditer(content):
            ...
            for field_match in field_pattern.finditer(body):
                ...
                struct.fields.append(StructField(name=name, datatype=datatype, offset=offset, size=size))
                offset += size
...
def create_json_export(parser: KotorREParser, output_path: str):
    ...
    data = {
        ...
        "data_definitions": [
            {"address": d.address, "address_hex": f"0x{d.address:08x}", "datatype": d.datatype, "namespace": d.datatype_namespace, "size": d.size}
            for d in parser.data_definitions
        ],
        "comments": [{"address": c.address, "address_hex": f"0x{c.address:08x}", "type": c.comment_type, "text": c.text} for c in parser.comments],
        "structs": [
            {
                "name": s.name,
                "size": s.size,
                "namespace": s.namespace,
                "fields": [{"name": f.name, "datatype": f.datatype, "offset": f.offset, "size": f.size} for f in s.fields],
            }
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(description="Parse KOTOR RE Things and generate Ghidra import data")
    arg_parser.add_argument("--input", "-i", default="vendor/Kotor RE Things", help="Path to KOTOR RE Things directory")
    arg_parser.add_argument("--output-script", "-s", default="scripts/ghidra_kotor_apply.py", help="Output path for Ghidra Python script")
    arg_parser.add_argument("--output-json", "-j", default="kotor_re_full.json", help="Output path for JSON export")
    arg_parser.add_argument("--json-only", action="store_true", help="Only generate JSON, skip Ghidra script")
