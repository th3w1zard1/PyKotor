...
$toolPath = (Resolve-Path -LiteralPath "$rootPath/Tools/GuiConverter").Path
...
$argsList = @(
    "--tool-path", $toolPath
    "--entrypoint", "gui_converter/__main__.py"
    "--name", "GuiConverter"
...
if ((Get-LocalOS) -eq "Windows") {
    $argsList += @("--pre-pip", "comtypes")
}
