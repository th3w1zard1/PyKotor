...
def main():
    ...
    if match:
        print(f"✓ PASS: Unpacked file matches reference")
        print(f"  {analysis}")
    else:
        print(f"NOTE: Unpacked file differs from reference (likely build/version differences)")
        print(f"  This is expected if Steam version and reference are different builds")
    ...
    if match:
        print(f"PASS: Pack -> Unpack restored original file perfectly")
        print(f"  {analysis}")
    else:
        print(f"FAIL: Pack -> Unpack did not restore original file")
        print(analysis)
        print()
    ...
    if hash_packed1 == hash_packed2:
        print(f"✓ PASS: Idempotency test passed")
        print(f"  Pack -> Unpack -> Pack produces identical files")
    else:
    ...
    print(f"  ✓ Packer/unpacker is fully idempotent")
