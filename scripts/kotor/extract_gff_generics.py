...
# Read the full file
lines: list[str] = gff_file.read_text(encoding="utf-8").splitlines()

# Create output files for each section
section_names: list[str] = list(section_starts.keys())
section_names.append("END")  # Add end marker

for i, section_name in enumerate(section_names[:-1]):
    start_line: int = section_starts[section_name] - 1  # Convert to 0-based index
    next_section: str = section_names[i + 1]

    if next_section != "END":
        end_line: int = section_starts[next_section] - 1
    else:
        end_line = len(lines)  # End of file

    # Extract section content
    section_lines: list[str] = lines[start_line:end_line]
    section_content: str = "".join(section_lines)
    ...
    lines_list: list[str] = section_content.split("\n")
    ...
    output_file: Path = wiki_dir / filename
