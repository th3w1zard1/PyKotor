...
if command -v python3 &> /dev/null; then
    ...
    python3 -m pytest src/hologenerator/tests/ -v --tb=short || echo "Python tests not yet implemented"
    
    echo "Running CLI tests..."
    python3 -m unittest src/hologenerator/tests/test_cli.py -v || echo "CLI tests not yet implemented"
...
if command -v npm &> /dev/null; then
    ...
    npm test -- --coverage --watchAll=false || echo "React tests not yet fully implemented"
