...
def test_utc_editor_manipulate_firstname_locstring(qtbot: QtBot, installation: HTInstallation, test_files_dir: Path):
...
def test_utceditor_editor_help_dialog_opens_correct_file(qtbot, installation: HTInstallation):
    ...
    assert "Help File Not Found" not in html, f"Help file 'GFF-UTC.md' should be found, but error was shown. HTML: {html[:500]}"
...
# ============================================================================
# REFERENCE SEARCH FUNCTIONALITY TESTS
# ============================================================================
...
def test_utc_editor_reference_search_context_menu_script_field(qtbot, installation: HTInstallation):
    ...
    # Get context menu for script field
    menu = editor.ui.onHeartbeatSelect.createStandardContextMenu()
    if menu is None:
...
def test_reference_finder_find_script_references(qtbot, installation: HTInstallation):
    ...
    # Search for a common script (should find at least some results in a real installation)
...
def test_file_results_dialog_with_reference_results(qtbot, installation: HTInstallation):
    ...
    # We need a real file resource, so let's try to get one from the installation
    test_resource = None
    ...
    if test_resource is None:
        pytest.skip("No UTC files found in installation for testing")

    result = ReferenceSearchResult(
        file_resource=test_resource,
