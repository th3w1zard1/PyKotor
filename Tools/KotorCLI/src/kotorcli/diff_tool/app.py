#!/usr/bin/env python3
"""Main application logic for KotorDiff (integrated into KotorCLI).
...
def log_output(*args, **kwargs):
    ...
    try:
        ...
        try:
            ...
            print(safe_msg, **kwargs)  # noqa: T201
        except Exception:
        ...
        if not _global_config.output_log.parent.is_dir():
            while True:
                ...
                if _global_config.output_log.parent.is_dir():
                    break
                print("Invalid path:", _global_config.output_log)  # noqa: T201

    # Write the captured output to the file (always use UTF-8 for file)
    with _global_config.output_log.open("a", encoding="utf-8") as file_handle:
...
def _import_logging_system():
    ...
    """
    try:
        from kotorcli.diff_tool.logger import LogLevel, OutputMode, setup_logger  # noqa: PLC0415
    except ImportError:
...
def _setup_logging(config: KotorDiffConfig) -> None:
    ...
    """
    log_level, output_mode, setup_logger = _import_logging_system()
    if setup_logger is None or log_level is None or output_mode is None:
        # Logging system not available, skip setup
        return

    log_level = getattr(log_level, config.log_level.upper())
    output_mode = getattr(output_mode, config.output_mode.upper())
    ...
    if config.output_log_path:
        try:
            ...
            print(f"Warning: Could not open log file {config.output_log_path}: {exc}")  # noqa: T201
    ...
    if modifications.tlk and base_data_path:
        ...
        for tlk_mod in modifications.tlk:
            try:
                ...
                log_output(f"[Warning] StrRef analysis failed for tlk_mod={tlk_mod}: {exc.__class__.__name__}: {exc}")
                log_output(f"Full traceback (tlk_mod={tlk_mod}):")
                for line in traceback.format_exc().splitlines():
                    log_output(f"  {line}")

        log_output("StrRef analysis complete. Added linking patches:")
        log_output(f"  GFF patches: {sum(len(modifier.modifiers) for modifier in modifications.gff)}")
        log_output(f"  2DA patches: {sum(len(modifier.modifiers) for modifier in modifications.twoda)}")
        log_output(f"  SSF patches: {sum(len(modifier.modifiers) for modifier in modifications.ssf)}")
        log_output(f"  NCS patches: {sum(len(modifier.modifiers) for modifier in modifications.ncs)}")
...
def handle_diff_internal(
    ...
    *,
    ...
    """
    if len(files_and_folders_and_installations) < 2:
...
def handle_diff(config: KotorDiffConfig) -> tuple[bool | None, int | None]:  # noqa: PLR0912, PLR0915
        ...
        if config.use_incremental_writer:
            ...
            if base_path is not None:
                try:
                    ...
                    log_output(f"[Warning] Could not determine game: {exc.__class__.__name__}: {exc}")
            ...
            # Structure: {installation_index: CaseInsensitiveDict[TwoDAMemoryReferenceCache]}  # noqa: ERA001
