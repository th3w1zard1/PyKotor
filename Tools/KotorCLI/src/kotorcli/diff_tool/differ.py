...
class KotorDiffer:
    ...
    def _2da_to_text(self, twoda_obj: twoda.TwoDA) -> str:
        ...
        # Rows
        for row_index, row in enumerate(twoda_obj.rows):
            row_data = [str(row_index)]
    ...
    def _lip_to_text(self, lip_obj: lip.LIP) -> str:
        ...
        for idx, keyframe in enumerate(lip_obj.keyframes):
            lines.append(f"Keyframe {idx}: Time={keyframe.time}, Shape={keyframe.shape}")
        return "\n".join(lines)
