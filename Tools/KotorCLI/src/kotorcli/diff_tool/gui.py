"""KotorDiff GUI application integrated with KotorCLI.

Provides the Tkinter UI, deferring to headless output when CLI arguments are
supplied.
"""
...
from utility.tkinter.base_app import ThemedApp

from kotorcli.diff_tool.app import KotorDiffConfig, run_application
...
class KotorDiffApp(ThemedApp):
    ...
    def initialize_ui_controls(self):
        ...
        self.path1_combobox.bind(
            "<<ComboboxSelected>>",
            lambda event: self.root.after(10, lambda: self.move_cursor_to_end(cast("ttk.Combobox", event.widget))),
        )
    ...
    def _run_diff_thread(self, should_cancel: Event):
        ...
        try:
            ...
            self._handle_general_exception(exc, "Error during diff operation")
        finally:
