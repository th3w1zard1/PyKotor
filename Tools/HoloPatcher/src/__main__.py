...
class MessageboxOverride:
    ...
    @staticmethod
    def askokcancel(title, message) -> bool | None:
        """Console-based replacement for messagebox.askyesno and similar."""
        print(f"{title}\n{message}")  # noqa: T201
        while True:
            response = input("(y/N)").lower().strip()
...
class UIApp:
    ...
    def initialize_top_menu(self):
        ...
        tools_menu.add_command(label="Validate INI", command=self.child.test_reader)
        tools_menu.add_command(label="Uninstall Mod / Restore Backup", command=self.child.uninstall_selected_mod)
        tools_menu.add_command(label="Fix permissions to file/folder...", command=self.child.fix_permissions)
        tools_menu.add_command(label="Fix iOS Case Sensitivity", command=self.child.lowercase_files_and_folders)
        tools_menu.add_command(label="Create info.rte...", command=self.child.create_rte_content)
        ...
        about_menu.add_command(label="Check for Updates", command=self.child.check_for_updates)
    ...
    def initialize_ui_controls(self):
        ...
        ToolTip(self.namespaces_combobox, self.child.get_namespace_description)
        self.namespaces_combobox.bind("<<ComboboxSelected>>", self.child.on_namespace_option_chosen)
        ...
        self.browse_button: ttk.Button = ttk.Button(top_frame, text="Browse", command=self.child.open_mod)
        self.browse_button.grid(row=0, column=1, padx=5, pady=2, sticky="e")
        self.expand_namespace_description_button: ttk.Button = ttk.Button(top_frame, width=1, text="?", command=lambda *args: messagebox.showinfo(self.namespaces_combobox.get(), self.child.get_namespace_description(*args)))
        ...
        self.gamepaths.bind("<<ComboboxSelected>>", self.child.on_gamepaths_chosen)
        # Browse for a KOTOR path
        self.gamepaths_browse_button = ttk.Button(top_frame, text="Browse", command=self.child.open_kotor)
        ...
        self.exit_button = ttk.Button(bottom_frame, text="Exit", command=self.child.handle_exit_button)
        self.exit_button.pack(side="left", padx=5, pady=5)
        self.install_button = ttk.Button(bottom_frame, text="Install", command=self.child.begin_install)
    ...
    def on_gamepaths_chosen(
        ...
        self.ui.root.after(10, lambda: self.move_cursor_to_end(event.widget))
    ...
    def get_namespace_description(self) -> str:
        ...
        namespace_option: PatcherNamespace | None = next(
            (x for x in self.child.namespaces if x.name == self.namespaces_combobox.get()),
            None,
        )
...
class App():
    def __init__(self):
        self.ui: UIApp = UIApp()
        self.ui.root.title(f"HoloPatcher {VERSION_LABEL}")
        ...
        self.ui.root.protocol("WM_DELETE_WINDOW", self.handle_exit_button)
    ...
    def execute_commandline(
        ...
        if cmdline_args.namespace_option_index:
            self.ui.namespaces_combobox.set(self.ui.namespaces_combobox["values"][cmdline_args.namespace_option_index])
    ...
    def uninstall_selected_mod(self):
        ...
        fully_ran: bool = True
        try:
            uninstaller = ModUninstaller(backup_parent_folder, Path(self.ui.gamepaths.get()), self.logger)
        ...
        if not fully_ran:
            self.on_namespace_option_chosen(object())
    ...
    def handle_exit_button(self):
        ...
        if (
            ...
            return
        if self.ui:
            self.ui.simple_thread_event.set()
            time.sleep(1)
    ...
    def lowercase_files_and_folders(
        ...
        directory = directory or self.ui.filedialog.askdirectory()
        if not directory:
            return
    ...
    def on_namespace_option_chosen(
        ...
        try:
            ...
            namespace_option: PatcherNamespace = next(x for x in self.namespaces if x.name == self.ui.namespaces_combobox.get())
            ...
            if self.ui:
                self.ui.root.after(10, lambda: self.ui.move_cursor_to_end(self.ui.namespaces_combobox))
    ...
    def load_namespace(
        ...
        """
        self.ui.namespaces_combobox["values"] = namespaces
        self.ui.namespaces_combobox.set(self.ui.namespaces_combobox["values"][0])
        self.namespaces = namespaces
        self.on_namespace_option_chosen(object(), config_reader)
    ...
    def open_mod(
        ...
        try:
            ...
            if default_directory_path_str:
                self.ui.browse_button.place_forget()
            if not namespace_path.safe_isfile():
                self.ui.namespaces_combobox.place_forget()
    ...
    def open_kotor(
        ...
        try:
            ...
            self.ui.gamepaths.set(str(directory))
            if directory_str not in self.ui.gamepaths["values"]:
                self.ui.gamepaths["values"] = (*self.ui.gamepaths["values"], directory_str)
            self.ui.root.after(10, self.ui.move_cursor_to_end, self.ui.namespaces_combobox)
        except Exception as e:  # pylint: disable=W0718  # noqa: BLE001
    ...
    def preinstall_validate_chosen(self) -> bool:
        ...
        if not self.mod_path or not CaseAwarePath(self.mod_path).safe_isdir():
            ...
            return False
        game_path: str = self.ui.gamepaths.get()
        if not game_path:
            messagebox.showinfo(
        ...
        self.ui.gamepaths.set(game_path_str)
    ...
    def begin_install(self):
        ...
        try:
            if not self.preinstall_validate_chosen():
                return
            self.task_thread = Thread(target=self.begin_install_thread, args=(self.ui.simple_thread_event,))
            self.task_thread.start()
        except Exception as e:  # pylint: disable=W0718  # noqa: BLE001
    ...
    def begin_install_thread(self, should_cancel_thread: Event | None = None):
        ...
        self.clear_main_text()
        try:
            installer = ModInstaller(namespace_mod_path, self.ui.gamepaths.get(), ini_file_path, self.logger)
    ...
    def set_state(
        ...
        if state:
            ...
            self.ui.install_button.config(state=tk.DISABLED)
            self.ui.gamepaths_browse_button.config(state=tk.DISABLED)
            self.ui.browse_button.config(state=tk.DISABLED)
        else:
            self.task_running = False
            self.initialize_logger()  # reset the errors/warnings etc
            self.ui.install_button.config(state=tk.NORMAL)
            self.ui.gamepaths_browse_button.config(state=tk.NORMAL)
            self.ui.browse_button.config(state=tk.NORMAL)
    ...
    def _execute_mod_install(
        ...
        should_cancel_thread: Event | None = None,
    ):
